...
- Add-ons (one-time bumps for the current period):
  - `EXTRA_POSTS_100`: +100 posts
  - `EXTRA_SCHEDULES_100`: +100 scheduled posts
  - `EXTRA_AI_50`: +50 AI requests

### Frontend tasks
- Add “Buy” flows for add-ons when a limit is reached; call the token API with `itemType: "addon"` + `addonCode`.
